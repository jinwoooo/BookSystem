 ...
 */
	...
	// 수정내용: 레이아웃을 설정하지않으면 텍스트필드가 아주 작게 나오는 현상이 있었는데, 처음 텍스트필드를 생성할때, 크기를 선언하지 않아서 생긴 문제였음. 크기 선언하고 고쳐짐.
	...
	private void LoadData_Table_Title(String bookTitle) {
		...
		if (bookTitle.length() != 0){
			...
			for(int i = 0; i < titlecatalog[(int) bookTitle.charAt(0)].size();i ++){
				...
				if(	bookTitle.equals(tempTitle)){	//책제목이 일치
	...
	private void ReturnBook() {
		...
		if( T_Return_bookTitle.getText().length() != 0 &&	//3개의 입력필드 모두에 값이 들어왔다.
			...
			int hasBook = 0;
			//책제목과 저자명으로 해당 책이 존재하는지에 대한 체크가 필요.
			hasBook = titlecatalog[(int) T_Return_bookTitle.getText().charAt(hasBook)].size();
